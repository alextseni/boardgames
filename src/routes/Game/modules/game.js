...
const ACTION_HANDLERS = {
  [START_STATE]: (state, action) => {
    ...
    return (Object.assign({},
            { pieces: p, allMarbles: p.filter((m) => m.type === 'marble').length },
            action.payload));
  },
  ...
  [PLAYER_STATE]: (state, action) => {
    ...
    : action.payload.tag === 'reset' ? p = state.pieces.map((p) => p.type === 'selected'
                                        ? { ...p, type: 'marble' } : p)
    : false;
  ...
  [BOARD_STATE]: (state, action) => {
    let correct = false;
    const countX = Object.keys(_.countBy(
                               _.filter(state.pieces, (o) => o.type === 'selected'), (o) => o.x));
    const countY = Object.keys(_.countBy(
                               _.filter(state.pieces, (o) => o.type === 'selected'), (o) => o.y));

    const evaluate = (counter, stop, axis, m, n) => {
      correct = !state.pieces.find((p) => p[m] == counter && p[n] == axis[0] &&
                                  (p.type === 'obstacle' || p.type === 'marble'));
      (!correct || counter==stop) ? 0 : evaluate(--counter, stop, axis, m,n);
      return;
    };

    (countY.length === 1) ? evaluate(countX[countX.length - 1], countX[0], countY, 'x', 'y')
    : false;
    (countX.length === 1) ? evaluate(countY[countY.length - 1], countY[0], countX, 'y', 'x')
    : false;
    ...
    const result = [
      ...
      },
      {
        ...
        is: state.pieces.filter(p => (p.type === 'marble')).length === 1,
      },
      {
