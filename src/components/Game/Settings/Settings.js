...
export const Settings = ({ game, changeDifficulty, changeBoard, initializeBoard }) => {
  ...
  return (
    ...
    <p className={classes.label}>Board:</p>
      ...
      >
        <MenuItem style = {styles.menuItem} onClick={()=> {changeBoard(4); initializeBoard(game.options.size);}} value={4} primaryText="4x4" />
        <MenuItem style = {styles.menuItem} onClick={()=> {changeBoard(5); initializeBoard(game.options.size);}} value={5} primaryText="5x5" />
        <MenuItem style = {styles.menuItem} onClick={()=> {changeBoard(6); initializeBoard(game.options.size);}} value={6} primaryText="6x6" />
      </DropDownMenu>
