...
export const GameInfo = ({ props }) => {
  const bar = Math.round((100 - (((props.game.pieces.filter((p) => p.type === 'marble' ||
              p.type === 'selected').length - 1)) / (props.game.allMarbles - 1)) * 100));
  return (
    <div className={classes.info}>
      <span className={'label label-default'} id={classes[props.game.text.substring(0, 7)]}>
        {props.game.text}
      </span>
