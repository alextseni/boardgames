...
const ACTION_HANDLERS = {
  [LOGIN_STATE]: (state, action) => {
    ...
    if (state.user.flag=='in') {flag='in'; message={fail: 'Already logged in!'}}
    return {
    ...state,
    onlineUsers: upUsers,
  ...
  [LOGOUT_STATE]: (state, action) => ({
    ...
    activeGames: _.remove(state.activeGames,(a)=> {state.user.name === a.user.name}),
  }),
...
[HOST_STATE]: (state, action) => {
  if (state.user.flag != 'in') {return state;}
  let isProtected = 'yes';
  if (action.payload == '') {
    isProtected = 'no';
  }
  ...
  activeGames: _.concat(state.activeGames, [{user:state.user,pwd:action.payload, isProtected: isProtected}]),
  }
},
...
// Reducer
// ------------------------------------
const initialState = {allUsers: [], message: {}, onlineUsers: [], activeGames: [], user:{flag:'', reserved: 'no',}};
